<?php
	class T_99999_M_FLOOR {
		...
		public function getList() {
			...
			$stmt = $pdo->query('SELECT * FROM '.get_class($this).' ORDER BY 2 ASC');
			return $stmt;
		}
		...
		public function getNewSortNo() {
			$pdo = ConnDB::getInstance();
			$stmt = $pdo->query('SELECT max(cast(FIELD002 as signed)) + 1 AS FIELD002 FROM '.get_class($this));
			$f002 = "";
			while($row = $stmt->fetch(PDO::FETCH_NUM)) {
				$f002 = htmlspecialchars($row[0]);
			}
			return $f002;
		}
